...
사용법:  cd ~/stock-news-bot && python3 일정_특징주_섹션_패치_v3.py
"""
...
HELPER_CODE = r'''"""일정 알림용 특징주/상한가 섹션 (patch: 일정_특징주_섹션_패치_v3).
...
_NOT_LIMIT_UP = ("근접", "직전", "앞두", "불발", "실패", "못 ", "못가", "하한가")
...
def main() -> None:
    ...
    for target in candidates:
        ...
        if MARKER in src:
            RESULTS.append(f"{target.relative_to(root)}: 이미 패치되어 있음")
            print(f"ℹ️ {target.name}: 이미 패치되어 있음(건너뜀)")
            patched = True
            continue
        if "format_featured_digest_lines" in src or "특징주 누적 랭킹" in src:
            RESULTS.append(f"{target.relative_to(root)}: 기존 특징주 누적 랭킹 코드가 있어 수정 안 함")
        ...
        if problems:
            RESULTS.append(f"{target.relative_to(root)}: 위치 확정 실패 → " + " / ".join(problems) + f" (기준줄 {anchor_line})")
        ...
        if kind == "list":
            ...
            add = [f"{ind}    {var} += \"\\n\" + _feat_text + \"\\n\""]
        block = [
            f"{ind}# {MARKER} 일정 알림 특징주 섹션 (일정_특징주_섹션_패치_v3)",
            f"{ind}try:",
        ...
        try:
            ...
            continue
        RESULTS.append(f"{target.relative_to(root)}: 삽입 성공 ({stmt.lineno}번째 줄 앞)")
