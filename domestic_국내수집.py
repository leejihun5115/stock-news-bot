...
DOMESTIC_RSS_URLS = [
    ...
    # [수정] 한경 직접 RSS(https://www.hankyung.com/feed/all-news)가 헤더를
    # Referer/Accept-Language까지 갖춰도 계속 403을 반환해(IP/WAF 단위 차단으로
    # 추정) 제거했다. 우리 서버가 hankyung.com에 직접 요청하지 않고 Google이
    # 이미 크롤링해둔 결과를 가져오는 구글뉴스 검색 RSS로 대체해 같은 차단을
    # 우회한다.
    "https://news.google.com/rss/search?q=site:hankyung.com&hl=ko&gl=KR&ceid=KR:ko",
...
DOMESTIC_RSS_SOURCE_NAMES = {
    ...
    "https://news.google.com/rss/search?q=site:hankyung.com&hl=ko&gl=KR&ceid=KR:ko": "한국경제(구글경유)",
...
def _engine_run_google_and_domestic():
    ...
    if ENABLE_DOMESTIC_NEWS:
        for url in DOMESTIC_RSS_URLS:
            ...
            for e in entries[:50]:
                title = e.get("title", "")
                link = e.get("link", "")
                # [진단용] 다운스트림(콜드스타트/번역/MASTER 등) 판정과 무관하게,
                ...
                if _engine_process_item(source, title, link, _engine_entry_published(e), e.get("summary", "")):
                    source_new += 1
                    total += 1
            first_title = _engine_clean(entries[0].get("title", ""))[:60] if entries else ""
            first_pub = _engine_entry_published(entries[0]) if entries else ""
            _engine_log("info", "[국내RSS 상세] %s | 원본=%d건 | 미확인=%d건 | 신규전송=%d건 | 최신항목=%s (%s)",
                        source, len(entries), unseen, source_new, first_title or "없음", first_pub or "발행시각 없음")
    else:
    ...
    if ENABLE_US_NEWS:
        for url in US_RSS_URLS:
            ...
            first_title = _engine_clean(entries[0].get("title", ""))[:60] if entries else ""
            first_pub = _engine_entry_published(entries[0]) if entries else ""
            _engine_log("info", "[해외RSS 상세] %s | 원본=%d건 | 미확인=%d건 | 신규전송=%d건 | 최신항목=%s (%s)",
                        url[:50], len(entries), unseen, source_new, first_title or "없음", first_pub or "발행시각 없음")
