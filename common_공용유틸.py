...
# ============================================================
# [로그 폭주 방지] 동일 오류 반복 억제 (Error Log Deduplication)
# ------------------------------------------------------------
# [문제] 기존 log_error()는 호출될 때마다 무조건 전체 상세 로그를 찍는다.
# 파일 수정으로 특정 함수에 버그 하나가 생기면(예: DART 수집 단계), 그 버그가
# 60초 주기마다 "완전히 동일한" 오류를 영원히 반복 발생시킨다. 근본 원인은
# 1개인데 로그창에는 수백~수천 줄이 쌓여서 "로그 에러가 많이 생겨서 뭐가
# 문제인지 알 수 없는" 상태가 된다.
# [해결] 같은 (context, 예외타입, 예외메시지) 조합이 짧은 시간 안에 반복되면,
# 최초 1회만 전체 상세(원인/위치)를 출력하고, 이후 반복은 카운트만 누적하다가
# 5회/20회/100회 등 특정 지점에서만 "N회째 반복 중"이라는 요약 한 줄만 남긴다.
# 새로운 종류의 오류(예: 다른 파일에서 새로 생긴 버그)는 즉시 상세히 찍히므로
# 진짜 새로운 문제를 놓치지 않는다.
# ============================================================
ERROR_DEDUP_WINDOW_SEC = int(os.environ.get("NEWS_BOT_ERROR_DEDUP_WINDOW_SEC", "600"))  # 10분
_ERROR_DEDUP_CACHE = {}
_ERROR_DEDUP_LOCK = threading.Lock()
_ERROR_DEDUP_MAX_KEYS = 3000  # 서로 다른 오류 종류가 무한정 쌓이는 것을 막는 상한


def _error_dedup_key(context, exc):
    exc_sig = f"{type(exc).__name__}:{str(exc)[:160]}" if exc is not None else ""
    return f"{context}|{exc_sig}"


def _error_dedup_gate(context, exc):
    """반복 여부를 판단한다. 반환값: (지금 상세히 찍어야 하는가, 지금까지 누적 횟수)."""
    key = _error_dedup_key(context, exc)
    now = time.time()
    with _ERROR_DEDUP_LOCK:
        if len(_ERROR_DEDUP_CACHE) > _ERROR_DEDUP_MAX_KEYS:
            # 창이 만료된 것부터 정리(메모리 무한 증가 방지). 그래도 넘치면 가장 오래된 것부터 비운다.
            expired = [k for k, v in _ERROR_DEDUP_CACHE.items() if now - v["window_start"] >= ERROR_DEDUP_WINDOW_SEC]
            for k in expired:
                _ERROR_DEDUP_CACHE.pop(k, None)
            if len(_ERROR_DEDUP_CACHE) > _ERROR_DEDUP_MAX_KEYS:
                for k in list(_ERROR_DEDUP_CACHE.keys())[: len(_ERROR_DEDUP_CACHE) - _ERROR_DEDUP_MAX_KEYS]:
                    _ERROR_DEDUP_CACHE.pop(k, None)

        entry = _ERROR_DEDUP_CACHE.get(key)
        if entry is None or (now - entry["window_start"]) >= ERROR_DEDUP_WINDOW_SEC:
            _ERROR_DEDUP_CACHE[key] = {"window_start": now, "count": 1}
            return True, 1
        entry["count"] += 1
...
def log_error(context, exc=None, **details):
    ...
    함께 남긴다.
    [로그 폭주 방지] 완전히 동일한 오류가 짧은 시간 안에 반복되면 최초 1회만
    상세히 찍고, 이후에는 반복 횟수만 요약해 남긴다(_error_dedup_gate 참고).
    """
    should_log_full, count = _error_dedup_gate(context, exc)
    if not should_log_full:
        if count in (5, 20, 100) or count % 200 == 0:
            _logger.error(
                "[반복 오류 억제] %s | 최근 %d분 내 %d회째 동일 원인 반복 중(상세 로그는 최초 1회만 출력됨) "
                "| 원인이 파일 수정 직후부터라면 최근 변경사항부터 확인하세요.",
                context, ERROR_DEDUP_WINDOW_SEC // 60, count,
            )
        return
    parts = [f"[실패] {context}"]
