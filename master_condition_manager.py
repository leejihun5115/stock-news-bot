...
class MasterConditionManager:
    ...
    def _term_explanations(self, title, body):
        """정확한 전문용어만 설명한다. 회사명/기관명/고유명사는 용어로 취급하지 않는다.
        기사 문맥에서 정의가 확인되지 않으면 표시하지 않는다.
        """
        text = self._clean(f"{title} {body}")
        if not text:
            return []
        glossary = {
            "ETF": "여러 자산을 묶어 하나의 상품처럼 거래하는 상장지수펀드.",
            "VIX": "미국 주식시장의 향후 변동성에 대한 시장의 기대를 나타내는 지수.",
            "PER": "주가를 주당순이익으로 나눈 값으로, 이익 대비 주가 수준을 보는 지표.",
            "PBR": "주가를 주당순자산가치로 나눈 값으로, 순자산 대비 주가 수준을 보는 지표.",
            "ROE": "기업이 자기자본을 활용해 어느 정도의 이익을 냈는지 보여주는 지표.",
            "EPS": "기업의 순이익을 발행주식 수로 나눈 주당순이익.",
            "HBM": "여러 메모리 칩을 수직으로 연결해 높은 대역폭을 제공하는 고대역폭 메모리.",
            "IPO": "기업이 주식을 일반 투자자에게 처음 공개해 거래소에 상장하는 것.",
            "M&A": "기업의 인수 또는 합병을 뜻하는 표현.",
        }
        out=[]
    ...
    def _synthesize_title(self, title, body):
        """제목은 원문 우선. 본문 문장을 제목으로 승격하지 않는다.
        외국어 제목은 번역기가 제공한 한국어 제목이 이미 있으면 사용하고,
        없을 경우에도 원문을 임의로 본문에서 재구성하지 않는다.
        """
        title = self._clean(title)
        if not title:
            return ""
        # 편집용 메타/매체 꼬리표만 제거. 사건의 내용은 보존한다.
        title = re.sub(r"^\s*(?:Weekly Review|Market Update|Breaking|Analysis|주간 검토|시장 업데이트)\s*[|:：-]\s*", "", title, flags=re.I)
        title = re.sub(r"\s*[|｜-]\s*(?:The New Indian Express|The Economic Times|The Cool Down)\s*$", "", title, flags=re.I)
        return title.strip(" -–—|｜")
