...
def test_article_body_triggers_verification_call(monkeypatch):
    """article_body가 있으면 초안 작성 후 팩트체크 검수를 한 번 더 호출한다."""
    calls = []
    ...
    def fake_post(url, **kwargs):
        ...
        if "[검수 대상" in text:
            payload = '{"title":"","core":["500억원 공급계약(검수 통과)"],"analysis":["원문에서 확인된 내용만 남음"]}'
        else:
            payload = '{"title":"계약 확대","core":["500억원 공급계약"],"analysis":["계약 규모가 실적에 반영될 시점을 확인할 필요가 있다."]}'
    ...
    monkeypatch.setattr(llm_analyzer.requests, "post", fake_post)
    result = llm_analyzer.analyze_news(**{**_kwargs(), "article_body": "테스트 기업은 고객사와 500억원 규모의 공급계약을 체결했다고 공시했다."})

    assert len(calls) == 2  # 1단계 초안 + 2단계 팩트체크
    assert result.core == ["500억원 공급계약(검수 통과)"]
    assert result.title == "계약 확대"  # 검수 응답에 title이 없으면 초안 제목을 유지


def test_verification_failure_falls_back_to_draft(monkeypatch):
    """팩트체크 호출 자체가 실패해도 초안 결과는 그대로 살아있어야 한다."""
    call_count = {"n": 0}
    ...
    def fake_post(url, **kwargs):
        ...
        text = kwargs["json"]["contents"][0]["parts"][0]["text"]
        if "[검수 대상" in text:
            raise requests.Timeout("verify timeout")

        class FakeResponse:
            def raise_for_status(self):
                pass
            def json(self):
                return {"candidates": [{"content": {"parts": [{"text": '{"title":"계약 확대","core":["500억원 공급계약"],"analysis":["분석"]}'}]}}]}
        return FakeResponse()

    monkeypatch.setattr(llm_analyzer.requests, "post", fake_post)
    result = llm_analyzer.analyze_news(**{**_kwargs(), "article_body": "원문 본문"})

    assert call_count["n"] == 2
    assert result.title == "계약 확대"  # 검수 실패 -> 초안 그대로
