...
def _engine_cycle():
    ...
    engine_state_공유상태._engine_run_stage("국내/Google RSS 전체", _engine_run_google_and_domestic)
    engine_state_공유상태._engine_run_stage("번역 재시도 큐", _engine_retry_translation_queue)
    engine_state_공유상태._engine_run_stage("네이버 전체", _engine_run_naver)
    engine_state_공유상태._engine_run_stage("키워드 조합 전체", _engine_run_keyword_combinations)
    engine_state_공유상태._engine_run_stage("DART 전체", _engine_run_dart)
    engine_state_공유상태._engine_run_stage("텔레그램 채널 전체", _engine_run_telegram_channels)
    engine_state_공유상태._engine_run_stage("유튜브 전체", _engine_run_youtube)
    # 테스트는 부팅 시 1회만 송출한다. 일반 주기에서는 실행하지 않는다.
    engine_state_공유상태._engine_run_stage("국내장 장중 브리핑", _engine_krx_market_monitor)
