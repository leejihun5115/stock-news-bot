...
사용법:  cd ~/stock-news-bot && python3 일정_특징주_섹션_패치_v6.py
"""
...
HELPER_CODE = r'''"""일정 알림용 특징주/상한가 섹션 (patch: 일정_특징주_섹션_패치_v6).
...
def preview(root: Path, pkg: Path) -> None:
    ...
    total, shown = mod.upcoming_reliable(db)
    print(f"  예정 일정 {total}건 중 근거 확실한 {len(shown)}건만 표시")
    if text:
        print("\n" + text)
    else:
...
def plan_patch(src: str) -> dict:
    ...
    for line in lit_lines:
        ...
        for cand in cands:
            ...
            head = [
                f"{ind}# {MARKER} 일정 알림 특징주/상한가 섹션 (일정_특징주_섹션_패치_v6)",
                f"{ind}try:",
