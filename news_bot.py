# -*- coding: utf-8 -*-
"""
# 수정2 (2026-08-13) — 이전 "# 수정1" 버전 대비 업그레이드
#   ⚠️ 앞으로 수정할 때마다 이 번호를 올리고, 무엇이 바뀌었는지 여기 한 줄씩
#      추가해주세요. 버전이 헷갈리면 이 목록으로 "어디까지 반영됐는지" 확인.
#
#   [수정2에서 새로 추가/수정된 것]
#   - 🐛 약업신문/전자신문 중복 스팸 버그 수정 (초기화-실제체크 제목 계산 방식 통일)
#   - 🔗 해외뉴스 테마 감지 + 국내 관련주 자동 연결 (_detect_theme_from_text)
#   - 📊 미국기업 실적발표 beat/miss + 매출/EPS 구조화 표시 (_extract_earnings_info)
#   - 🆕 신규상장(IPO) 알림 - 상장예정일/공모가/업종/주력사업/연관테마
#   - ⏰ 특징주/급등종목 워치리스트 + DART 일정 D-7/D-3 리마인더
#   - 🌅 아침 브리핑 - 미국지수/테마판정/국내연결/대장주랭킹(상한가 우선)
#   - 🩺 국내/해외 RSS 진단 로그 (원본/이미본것/삭제어/오래됨 단계별 확인 가능)
#   - ✔️ 검은색 체크 아이콘 통일, 100점 중요도/재무점수, [요점], 신규·후속·재탕 구분
#   - 🔘 링크를 텍스트가 아니라 버튼으로 (확인창 문제 대응), http 자동 https 승격
#   - 🏢 종목 자동인식 통합 (종목코드/영문명/한글명 하나로)
#   - 🔧 extract_dart_schedule이 if __name__ 블록 뒤에 있어서 실제로 한 번도
#        정의된 적 없던 치명적 버그 수정 (블록 앞으로 재배치)
#
