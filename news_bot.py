...
# -*- coding: utf-8 -*-
"""
# 버그수정12 (2026-08-14) — 🔥🔥[매우 중대] DART 원문 조회가 거의 전부
...
THEME_DEFINITIONS = [
    ...
    },
    {
        ...
        "reasons": ["AI 데이터센터발 전력수요 급증", "전력설비 인프라 교체 주기"],
        "kr_stocks": ["HD현대일렉트릭", "효성중공업", "LS일렉트릭"],
    },
]
...
# 🕒 5일 전 뉴스 필터링 함수 추가
# ============================================================
def is_within_5_days(published_time):
    """기사 발행 시각이 현재 기준 5일 이내인지 확인"""
    if not published_time:
        return True  # 날짜 정보가 없으면 누락 방지를 위해 통과
    try:
        if isinstance(published_time, str):
            dt = parsedate_to_datetime(published_time)
            if dt.tzinfo:
                dt = dt.astimezone(_KST).replace(tzinfo=None)
        elif isinstance(published_time, datetime.datetime):
            dt = published_time
        else:
            return True
        
        diff = _now_kst() - dt
        return diff <= datetime.timedelta(days=5)
    except Exception:
        return True