...
# 📡 Telegram = 최종 송출 단말 (MASTER 65 FINAL_LOCK 소비 전용)
# 🗄️ JSONL/상태 DB = 기록실
#
# 권한 규칙: 자기 역할 밖에서 최종 판단/수정 금지.
# 최종 판단은 MASTER 65 한 곳에서만 확정하고, FINAL_LOCK 이후에는 읽기/표시/전송/기록만 수행한다.
# ============================================================
...
# 🔐 HIGHEST-LEVEL EXECUTION GATE
# 오직 MASTER 65의 FINAL_LOCK 결과만 실행권한을 가질 수 있다.
# 하위 모듈은 실행명령을 생성·변경·승격할 수 없다. 실패 시 항상 차단한다.
...
def _master_open_execution(result):
    ...
    ok = bool(
        ...
        and result.get("locked") is True
        and result.get("decision_owner") == "MASTER_65"
...
def _engine_schedule_daily_monitor():
    ...
    # 🔒 최고명령 전용: 일정 모니터는 자료만 준비한다. 송출 명령은 MASTER 65에서만 발급한다.
    return
...
# 🗄️ DB = 기록실: 최종 확정 결과의 감사(audit) 이력을 남긴다.
DECISION_AUDIT_DB = os.environ.get("NEWS_BOT_DECISION_AUDIT_DB", "news_bot_decision_audit.jsonl")
...
def _engine_watchdog_alert(force=False):
    ...
    # 🔒 WATCHDOG는 기록만 한다. Telegram 실행명령은 MASTER 65 외에서는 금지한다.
    return
...
def _engine_record_decision_audit(item, result, status="LOCKED"):
    """🗄️ DB = 기록실. 판단하지 않고 MASTER/Validator/FINAL_LOCK 결과만 기록한다."""
    try:
        row = {
            "ts_kst": _now_kst().isoformat(),
            "status": status,
            "source": str(item.get("source", "")),
            "title_raw": str(item.get("title", ""))[:500],
            "link": str(item.get("link", ""))[:1000],
            "decision_owner": str((result or {}).get("decision_owner", "MASTER_65")),
            "analysis_status": str((result or {}).get("analysis_status", "")),
            "locked": bool((result or {}).get("locked")),
            "final_lock_hash": str((result or {}).get("final_lock_hash", "")),
            "rule_version": str((result or {}).get("rule_version", "")),
            "related": [str(x.get("name", "")) for x in ((result or {}).get("related") or []) if isinstance(x, dict)],
            "leader": str(((result or {}).get("leader") or {}).get("name", "")),
            "stage": str((result or {}).get("stage", "")),
            "news_value": str((result or {}).get("news_value", "")),
        }
        return _engine_atomic_append_jsonl(DECISION_AUDIT_DB, row)
    except Exception as e:
        _engine_log("warning", "[DB] 판단기록 실패 | 원인=%s", str(e)[:160])
        return False


# 🔒 HIGHEST COMMAND ONLY:
# 아래 송출 함수는 MASTER_65_FINAL_LOCK을 소비하는 최종 단말일 뿐이며,
# 하위 함수/스케줄러/브리핑은 실행명령을 발급할 수 없다.

# 🔒 ROOT EXECUTION INVARIANT
# 이 파일에서 Telegram 실행은 오직 MASTER_65_FINAL_LOCK을 소비하는 최종 단말에서만 가능하다.
# 하위 모듈은 송출 함수/실행명령을 호출하거나 생성할 수 없다.
def _engine_send_telegram(text, *, command=None, action="NEWS_SEND", master_result=None):
    """최종 송출기.
    실행명령을 생성하지 않는다. 오직 MASTER 65의 MASTER_65_FINAL_LOCK만 소비한다.
    master_result가 없거나 명령이 정확히 일치하지 않으면 즉시 차단한다.
    """
    approved = bool(
        _MASTER_EXECUTION_AUTH
        and _MASTER_EXECUTION_COMMAND == "MASTER_65_FINAL_LOCK"
        and isinstance(master_result, dict)
        and master_result.get("locked") is True
        and master_result.get("decision_owner") == "MASTER_65"
        and master_result.get("execution_authorized") is True
        and master_result.get("execution_command") == "MASTER_65_FINAL_LOCK"
        and command == "MASTER_65_FINAL_LOCK"
    )
    if not approved:
        _engine_log("error", "[차단] MASTER 65 FINAL_LOCK 실행명령 없는 하위 송출")
        return False
    if not BOT_TOKEN or not CHAT_ID:
    ...
    try:
        ...
        if r.ok and api_result.get("ok", True):
            _engine_log("info", "[성공] Telegram 전송 | MASTER 65 FINAL_LOCK 승인")
            return True
...
def _engine_external_time_gate(source, published, title, extra, market_state, market_hits):
    ...
    age = (_now_kst() - dt).total_seconds()
    # 장중: 최근 60분만 기본 허용.
    # 장외/마감후: 다음 장 개장 전까지 강한 시장재료만 허용.
    # 시장 상태를 확인할 수 없는 경우에는 기존의 보수적인 60분 기준을 유지한다.
    if age <= 3600:
        return True, "최근60분"
    if market_state not in ("시장 마감 후 뉴스", "시장 휴무로 미반영"):
        return False, "60분 초과"
    text = _engine_clean(f"{title} {extra}")
    text_l = text.lower()
...
def _engine_freshness(item):
    """🚫 LEGACY: 실행 금지. 신규/재탕 판정은 MASTER_65 결과만 사용한다."""
    raise RuntimeError("LEGACY freshness disabled: MASTER_65 only")
...
def _engine_duplicate_policy(item):
    """🚫 LEGACY: 실행 금지. 중복/재탕은 MASTER_65가 유일하게 판단한다."""
    raise RuntimeError("LEGACY duplicate policy disabled: MASTER_65 only")
...
def _engine_flush_pending():
    ...
    for item in locked_candidates[:ENGINE_MAX_SEND_PER_CYCLE]:
        ...
        if key in _engine_seen:
            continue
        # 🔒 중복/신규성 판단도 MASTER 명령 외에는 실행하지 않는다.
        # Legacy _engine_duplicate_policy/_engine_freshness는 표시·판단에 사용하지 않는다.
        ...
        master_badge = _engine_master_badge(master_result)
        image_sent = False
        news_command = master_result.get("execution_command")
...
def _engine_process_item(source, title, link, published="", extra=""):
    ...
    # 시간대별 송출 규칙: 장중에는 최근 60분을 우선 적용하고, 장 마감 후에는
    # 다음 장 개장 전까지 시장반영 가능성이 있는 신규/강한 재료를 MASTER 65로 넘긴다.
    # 단순히 오래됐다는 이유만으로 장외 중요 뉴스를 전부 폐기하지 않는다.
...
def _engine_krx_market_monitor():
    ...
    # 🔒 국내장 브리핑은 자료 수집만 한다. 직접 송출하지 않는다.
...
def _engine_us_market_monitor():
    ...
    if slot_index == 1:
        ...
        if not msg:
            _US_BRIEFING_LAST_SNAPSHOT = snapshot
            return
    # 🔒 미국장 브리핑은 자료 수집만 한다. 직접 송출하지 않는다.
