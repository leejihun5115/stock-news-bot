...
def _calculate_importance_100(matched_count, is_exclusive, is_breaking, is_feature,
    ...
    # 상용화 등급 체계: S는 사용하지 않고 A+를 최고등급으로 통일
    if score >= 90:
        grade = "A+"
    elif score >= 80:
        grade = "A"
...
# ------------------------------------------------------------
# 원칙
# 1. 기존 수집/전송 로직을 깨지 않는다.
# 2. 제목/공시 요약에서 확인 가능한 정보만 점수화한다.
# 3. 확인하지 못한 주가/수급은 추정하지 않는다.
# 4. 최고 등급은 A+ 하나만 사용한다.
# ============================================================

COMMERCIAL_GRADE_THRESHOLDS = ((90, "A+"), (80, "A"), (70, "B"), (60, "C"), (0, "D"))


def _commercial_grade(score):
    score = max(0, min(100, int(round(score))))
    for threshold, grade in COMMERCIAL_GRADE_THRESHOLDS:
        if score >= threshold:
            return grade
    return "D"


def _commercial_event_score(title, highlight_suffix=""):
    text = f"{title} {highlight_suffix}"
    rules = [
        (25, ("FDA", "품목허가", "임상3상", "세계최초", "독점")),
        (23, ("대규모 수주", "공급계약", "수주", "장기공급계약", "대형계약")),
        (22, ("흑자전환", "영업이익 급증", "최대실적", "어닝서프라이즈")),
        (20, ("인수", "합병", "M&A", "경영권")),
        (18, ("증설", "양산", "신규고객", "고객사 확대")),
        (15, ("국책과제", "정부정책", "수혜", "기술개발")),
    ]
    score = 0
    matched = []
    for points, words in rules:
...
def _commercial_numeric_score(title, highlight_suffix=""):
    text = f"{title} {highlight_suffix}"
    score = 0
    values = []
    for m in re.finditer(r"([0-9]+(?:\.[0-9]+)?)\s*(조원|억원|억|조|%)", text):
        try:
    ...
    for m in re.finditer(r"([0-9]+(?:\.[0-9]+)?)\s*%", text):
        try:
            pct_values.append(abs(float(m.group(1))))
        except Exception:
            pass
    if pct_values:
        p = max(pct_values)
        score += 5 if p >= 10 else 3
        if p >= 50:
            score += 5
    return min(score, 20)


def calculate_commercial_score(title, highlight_suffix="", novelty="신규",
                               is_exclusive=False, is_breaking=False, is_feature=False,
                               is_disclosure=False):
    """상용화용 100점 재료 평가. 데이터가 없는 항목은 0점으로 두며 추정하지 않는다."""
    text = f"{title} {highlight_suffix}"
    event_score, event_hits = _commercial_event_score(title, highlight_suffix)
    numeric_score = _commercial_numeric_score(title, highlight_suffix)

    company_score = 10 if resolve_companies_in_text(title) else 0
    tag_score = (5 if is_exclusive else 0) + (4 if is_breaking else 0) + (3 if is_feature else 0)
    novelty_score = 5 if novelty == "신규" else (2 if novelty == "후속" else 0)

    positive_words = ("증가", "급증", "확대", "돌파", "상회", "수혜", "성공", "승인", "확보")
    risk_words = ("소송", "횡령", "배임", "감사의견", "거래정지", "상장폐지", "유상증자", "전환사채")
    positive_score = min(8, sum(1 for w in positive_words if w in text) * 2)
    risk_score = min(20, sum(1 for w in risk_words if w in text) * 5)

    total = event_score + numeric_score + company_score + tag_score + novelty_score + positive_score
    total = max(0, min(100, total))
    # 위험은 별도 표시. 위험이 있다고 해서 호재점수를 임의로 깎지 않는다.
    risk = risk_score
    final = max(0, min(100, total - int(risk * 0.5)))

    return {
        "score": total,
        "grade": _commercial_grade(total),
        "final_score": final,
        "final_grade": _commercial_grade(final),
        "risk_score": risk,
...
def _commercial_score_line(result):
    hits = ", ".join(result.get("event_hits", [])[:3])
    line = f"🏆 상용화판정 : {result['final_grade']} ({result['final_score']}/100)"
    line += f"\n📊 재료 {result['score']}/100 · 위험 {result['risk_score']}/100"
    if hits:
        line += f"\n🎯 핵심신호 : {hits}"
    return line
...
def send_telegram_message(title, news_url, time_str, matched_count, is_exclusive, is_breaking,
    ...
    # 🏆 상용화 엔진 V1: 기존 100점 점수와 별도로 재료/위험/최종판정을 계산한다.
    commercial_result = calculate_commercial_score(
        title, highlight_suffix, novelty, is_exclusive, is_breaking, is_feature, is_disclosure
    )
    commercial_line = _commercial_score_line(commercial_result)
    ...
    commercial_html = html.escape(commercial_line) + "\n"
...
#    바로 알려줍니다.
# ============================================================
def _rss_feed_status(feed):
    """RSS 상태를 안전하게 읽어 진단에 사용한다."""
    status = getattr(feed, "status", None)
    bozo = getattr(feed, "bozo", 0)
    bozo_exc = getattr(feed, "bozo_exception", None)
    return status, bozo, bozo_exc


def _rss_recent_article(entry):
    """RSS 날짜 파싱 실패로 정상 기사를 버리지 않도록 보수적으로 처리한다."""
    try:
        published = getattr(entry, "published", "")
        return is_recent_article(entry) or not published
    except Exception:
        return True


def _rss_should_send_domestic(title):
    """국내 RSS에서 종목명이 없어도 핵심 시장/산업 이벤트는 후보로 인정한다."""
    matched_count, is_exclusive, is_breaking, is_feature, should_send = classify_and_score(title)
    market_words = {
        "나스닥", "코스피", "코스닥", "금리", "연준", "FOMC", "CPI", "PCE",
        "반도체", "HBM", "AI", "인공지능", "데이터센터", "원전", "전력", "방산",
        "조선", "바이오", "임상", "배터리", "전기차", "유가", "원유", "금", "구리",
    }
    strong_market = any(w.lower() in title.lower() for w in market_words)
    return matched_count, is_exclusive, is_breaking, is_feature, (should_send or strong_market)


def check_domestic_news(current_time_str, max_sent_override=None):
    """국내 RSS 수집/필터/중복/신규성 파이프라인.

    기존 전송 로직은 유지하면서 RSS별 HTTP/bozo/원본/차단/오래됨/필터/재탕/전송
    진단을 강화해 운영 중 장애 원인을 한눈에 찾을 수 있게 한다.
    """
    feedparser.USER_AGENT = USER_AGENT
    totals = {"feed": 0, "entries": 0, "already": 0, "blocked": 0, "old": 0,
              "filtered": 0, "novelty_skip": 0, "sent": 0, "errors": 0}

    for rss_url in DOMESTIC_RSS_URLS:
        if max_sent_override and totals["sent"] >= max_sent_override:
            break

        source_label = DOMESTIC_RSS_SOURCE_NAMES.get(
            rss_url, urlparse(rss_url).netloc.replace("www.", "뉴스")
        )
        try:
            feed = _fetch_rss_feed(rss_url)
            totals["feed"] += 1
            status, bozo, bozo_exc = _rss_feed_status(feed)
            entries = list(getattr(feed, "entries", []) or [])
            totals["entries"] += len(entries)
            print(f"[🇰🇷 RSS진단] {source_label} | HTTP={status} | 원본={len(entries)} | bozo={bozo}")
            if bozo and bozo_exc:
                print(f"[🇰🇷 RSS경고] {source_label} | {bozo_exc}")
        except Exception as e:
            totals["errors"] += 1
            print(f"[국내 RSS 오류] {source_label} | {e}")
            continue

        local = {"already": 0, "blocked": 0, "old": 0, "filtered": 0,
                 "novelty_skip": 0, "sent": 0}
        ...
        for entry in entries:
            ...
            if is_blocked_title(title):
                ...
                mark_as_sent(title)
                continue
            if not _rss_recent_article(entry):
                local["old"] += 1
                mark_as_sent(title)
                continue

            matched_count, is_exclusive, is_breaking, is_feature, should_send = _rss_should_send_domestic(title)
            if not should_send:
                local["filtered"] += 1
                mark_as_sent(title)
                continue
            ...
            send_telegram_message(
                title, link, current_time_str, matched_count, is_exclusive, is_breaking,
                is_feature, False, source_label=source_label, novelty=novelty,
        ...
        for key, value in local.items():
            totals[key] += value
        print(
            f"[🇰🇷 RSS진단] {source_label} | 이미본={local['already']} | "
            f"삭제어={local['blocked']} | 오래됨={local['old']} | "
            f"필터탈락={local['filtered']} | 재탕={local['novelty_skip']} | "
            f"전송={local['sent']}"
        )

    print(
        f"[{current_time_str}] 국내 RSS 총괄: 소스={totals['feed']} "
        f"원본={totals['entries']} 이미본={totals['already']} 삭제어={totals['blocked']} "
        f"오래됨={totals['old']} 필터탈락={totals['filtered']} "
        f"재탕={totals['novelty_skip']} 전송={totals['sent']} 오류={totals['errors']}"
    )
...
def check_ipo_listings(current_time_str, bgn_date_override=None, max_sent_override=None):
    ...
    for item in data.get("list", []):
        ...
        # 🐛 DART 서버 속도제한 회피 (check_dart_disclosures와 동일한 조치)
        time.sleep(0.4)
...
def check_dart_disclosures(current_time_str, bgn_date_override=None, max_sent_override=None):
    ...
    while page_no <= max_pages:
        ...
        for item in data.get("list", []):
            ...
            scanned += 1
            # 🐛[버그 수정] 예전엔 테스트 모드(max_sent_override 지정됨)일 때만
            # 훑은 건수 상한이 있었는데, 정상 운영에서는 상한이 아예 없어서
            # 공시가 몰리는 날(예: 반기보고서 시즌 - 하루 300건+)엔 원문조회
            # 시도가 무제한으로 늘어나 사이클이 DART_CHECK_INTERVAL(1분)보다
            # 오래 걸릴 위험이 있었음. 정상 운영에도 합리적인 상한(150건)을
            # 둠 - 상한 넘긴 공시는 mark_as_sent를 안 해서 다음 사이클에
            # 자동으로 다시 시도됨(놓치지 않음).
            _scan_budget = (max_sent_override * 10) if max_sent_override else 150
            if scanned > _scan_budget:
            ...
            # 🐛[버그 수정] 공시가 많이 몰리는 날(예: 반기보고서 시즌)에 원문
            # 조회가 거의 전부 실패하는 문제가 있었음 - 재시도 로직은 있었지만
            # "여러 공시를 훑는 반복문 자체"에는 요청 사이 간격이 전혀 없어서,
            # 한꺼번에 수십~수백 건을 빠르게 두드리다가 DART 서버 속도제한에
            # 걸렸을 가능성이 높음. 요청 사이에 짧은 간격을 둬서 완화.
            time.sleep(0.4)
            report_text = _dart_document_text(rcept_no, timeout=30 if _is_large_dart_report(report_nm) else 15)
            if not report_text:
