...
def _engine_market_state(source, published):
    ...
    # [수정 2026-08-24] DART의 rcept_dt는 "20260824"처럼 날짜만 있고 시간 정보가
    # 없다. 이 문자열을 그대로 파싱하면 자정(00:00)으로 해석되어, 방금 접수된
    # 공시조차 장시간 범위(09:00~15:30) 밖으로 판정되어 "시장 마감 후 뉴스"로
    # 잘못 표시됐다(실제로는 장중에 막 들어온 공시였음). DART는 시간 정보가
    # 아예 없으므로, 발행 "날짜"가 오늘이면 시간 판정은 현재 KST 시각을
    # 기준으로 하고(=지금 막 들어온 것으로 취급), 날짜가 과거(백필 등)면
    # 기존처럼 확인불가로 둔다.
    if source == "DART":
...
def _engine_is_strong_news(item, master_result, badge_text=""):
    """[강한 뉴스 vs 일반 뉴스 분류 로직]
    ------------------------------------------------------------
    강한 뉴스(상세 분석 대상) 판정 기준 — 아래 중 하나라도 해당하면 강한 뉴스:
      ① 실적/경제지표 발표(컨센서스 존재) — earnings_info 추출됨
      ② 정책 결정(금리 등) / 공급망·수급 구조 변화 — _engine_strong_material 매칭
      ③ 과거 유사 사례와 매칭(historical_evidence) — 선례 비교 가능
      ④ MASTER가 news_value=='높음'으로 확정(🔥 강한 뉴스 배지)
    위 어느 것도 해당하지 않으면 일반 뉴스로 분류하여 한 줄 코멘트만 남긴다.
    (2026-08-24 합의: 강한 뉴스는 수치+과거대비+시나리오까지, 일반 뉴스는
    짧은 코멘트만.)
    """
    if '강한 뉴스' in (badge_text or ''):
        return True
    if (master_result or {}).get('historical_evidence'):
        return True
    if item.get('earnings_info') and item['earnings_info'][0]:
        return True
    strong, _hits = _engine_strong_material(item)
    return bool(strong)
...
def _engine_format_message(item):
    ...
    # 🔥 [강한 뉴스] vs 💬 [일반 뉴스] 분기
    # ------------------------------------------------------------
    # 강한 뉴스(실적/정책결정/수급구조 변화/과거사례 매칭)만 아래 🔎요약·
    # 🧠분석·🧠데이터 값 등 상세 섹션을 전부 표시한다. 그 외 일반 뉴스는
    # 여기서 바로 짧게 마무리한다(수치 나열·불릿 나열 없이 한 줄 코멘트만).
    # (2026-08-24 사용자 요청: "매매하는데 이런 값이 필요 있나" — 강한
    # 재료가 아닌 뉴스는 상세 분석 없이 짧게만 알려달라는 요청 반영)
    # ============================================================
    badge_text_early = str(_engine_master_badge(master_result) or '')
    is_strong = _engine_is_strong_news(item, master_result, badge_text_early)
    if not is_strong:
        one_liner = ''
        for candidate in (key_points or []) + ([analysis] if analysis else []) + list(outlook or []):
            candidate = re.sub(r'^[▶️•✔️\s]+', '', str(candidate)).strip()
            if candidate:
                one_liner = candidate
                break
        if one_liner:
            display = one_liner[:140] + ('…' if len(one_liner) > 140 else '')
    ...
    # [수정 2026-08-24] 예전엔 근거 없이 "🔥 강한 뉴스"라는 빈 라벨만 달랑
    # 표시됐다("매매하는데 이런 배지가 무슨 소용이냐"는 지적). 이제 실제 근거
    # 데이터(계약금액/매출액 대비 비율 등, DART 계약공시는
    # _dart_fetch_contract_detail이 채운 item['extra'])가 있으면 그 내용을
    # 그대로 보여주고, 근거가 전혀 없으면 라벨 자체를 표시하지 않는다.
    if '강한 뉴스' in badge_text and (master_result.get('news_value') == '높음' or master_result.get('historical_evidence')):
        contract_detail = str(item.get('extra') or '').strip()
        if contract_detail and ('계약금액' in contract_detail or '매출액 대비' in contract_detail):
            lines.append('🔥 <b>근거</b> : ' + html.escape(contract_detail[:220]))
        elif master_result.get('historical_evidence'):
            ratio = master_result.get('historical_match_ratio')
            ratio_txt = f" (유사도 {ratio:.0%})" if isinstance(ratio, (int, float)) else ""
            # [수정] "아래 데이터 값 참고"라고 안내했는데, 표본이 적어(hist/outc count<2)
            # 실제로는 그 섹션이 생략되는 경우 안내만 남고 내용이 없는 모순이 생겼다.
            # 섹션 존재를 전제로 하지 않는 자기완결 문장으로 바꾼다.
            lines.append(f'🔥 과거 유사 사례와 매칭됨{ratio_txt}')
        # 위 조건에 모두 해당하지 않으면(근거 데이터가 실제로 없으면) 표시하지 않는다.
    ...
    if lead_name:
        ...
        # [수정 2026-08-24] 표본이 1건뿐이면 비교 대상이 없어 매매 판단에 실질적
        # 도움이 안 된다는 피드백에 따라, 과거 이력이 최소 2건 이상 쌓였을 때만
        # 등장 이력을 보여준다. 1건짜리는 조용히 생략한다(형식적으로 '1건'만
        # 표시하는 것 자체가 노이즈였음).
        if hist and hist.get('count', 0) >= 2:
        ...
        # [수정] 등락 표본도 최소 2건 이상 쌓였을 때만 표시한다. 1건은
        # 상승비율/평균이 전부 그 한 건의 숫자로만 찍혀 의미가 없다.
        if outc and outc.get('count', 0) >= 2:
        ...
        if data_lines:
            # [데이터 표기 원칙] '최근'/'어제' 같은 상대 표현 대신 정확한 기준일을
            # 명시한다. 여기 표시되는 과거 이력/등락 통계는 이 기준일 시점 누적 DB 값이다.
            _data_asof = _now_kst().strftime('%Y-%m-%d')
            lines.append(f'🧠 <b>데이터 값</b> · {html.escape(lead_name)} 기준 ({_data_asof} 기준)')
...
def _engine_process_item(source, title, link, published="", extra=""):
    ...
    # [수정 2026-08-24] is_blocked_title()이 정의만 되고 실제로는 어디서도 호출되지
    # 않던 죽은 코드였다. 그 결과 "수상"처럼 이미 BLOCKED_KEYWORDS_BY_CATEGORY의
    # "🧹 시상·행사"에 등록된 키워드조차 실제로는 걸러지지 않고 그대로 송출됐다
    # (예: "...최고제품상 수상" 뉴스가 시세 무관인데도 그대로 나간 사례).
    # 번역 비용을 쓰기 전에 여기서 먼저 차단한다.
    if is_blocked_title(title):
        _engine_log("info", "[제외-차단키워드] %s | %s", source, title[:80])
        return False
    ...
    dt = _engine_parse_datetime(published)
    if dt:
        # [수정] DART는 시간 정보가 없어 자정으로 파싱되므로, 오늘 접수된 건이면
        # market_state와 동일하게 "지금" 시각을 표시한다(00:00으로 잘못 보이는 것 방지).
        if source == "DART" and dt.date() == _now_kst().date():
            dt = _now_kst()
