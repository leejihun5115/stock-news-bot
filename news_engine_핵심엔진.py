...
GLOBAL_AND_DOMESTIC_GIANTS = [
    ...
    "LIG넥스원", "KAI", "SK오션플랜트",
    # [확장] 반도체/장비
    "DB하이텍", "리노공업", "이오테크닉스", "티씨케이", "원익IPS", "solbrain", "솔브레인",
    "동진쎄미켐", "SK스퀘어", "SK아이이테크놀로지",
    # [확장] 2차전지/소재
    "포스코DX", "엘앤에프", "코스모신소재", "천보", "SKC", "LG이노텍",
    # [확장] 바이오/제약
    "셀트리온헬스케어", "유한양행", "삼성바이오에피스", "SK바이오사이언스", "메지온",
    "알테오젠", "레고켐바이오", "한올바이오파마",
    # [확장] 방산/조선/우주
    "현대로템", "HD현대중공업", "HD현대미포", "삼성중공업", "한화시스템", "쎄트렉아이",
    # [확장] IT/플랫폼/게임
    "크래프톤", "엔씨소프트", "넷마블", "펄어비스", "카카오게임즈", "더존비즈온",
    "카카오뱅크", "카카오페이", "네이버클라우드",
    # [확장] 자동차/부품
    "현대모비스", "만도", "한온시스템", "HL만도",
    # [확장] 금융/지주
    "KB금융", "신한지주", "하나금융지주", "우리금융지주", "삼성화재", "삼성증권",
    # [확장] 엔터/미디어
    "하이브", "JYP Ent", "에스엠", "와이지엔터테인먼트", "CJ ENM",
    # [확장] 통신/에너지
    "SK텔레콤", "KT", "LG유플러스", "한국전력", "한국가스공사", "두산퓨얼셀",
    # [확장] 원전/SMR
    "두산에너빌리티", "한전KPS", "한전기술",
]

# [버그 수정] 새 종목이 발견될 때마다 코드 수정→재배포를 하지 않아도 되도록,
# 관리자가 환경변수로 종목을 즉시 추가할 수 있게 한다. 예: NEWS_BOT_EXTRA_GIANTS="루닛,알에스오토메이션"
_EXTRA_GIANTS = [s.strip() for s in os.environ.get("NEWS_BOT_EXTRA_GIANTS", "").split(",") if s.strip()]
GLOBAL_AND_DOMESTIC_GIANTS = list(dict.fromkeys(GLOBAL_AND_DOMESTIC_GIANTS + _EXTRA_GIANTS))
