...
async def _health(_request: web.Request) -> web.Response:
    """프로세스뿐 아니라 Discord/파이프라인 상태까지 반영하는 헬스체크."""
    color, label, detail = _compute_state()
    # 부팅 중에는 Render가 콜드스타트 중인 서비스를 즉시 재시작하지 않도록 200을 유지한다.
    # READY 이후 stale/error 상태가 되면 503을 반환해 외부 모니터가 장애를 감지할 수 있게 한다.
    if bot_status.bot_ready and color == "red":
        return web.Response(status=503, text=f"unhealthy: {label} - {detail}")
...
def _compute_state() -> tuple[str, str, str]:
    ...
    stale_threshold = max(60, int(s.health_stale_threshold_seconds))
...
async def _status_page(_request: web.Request) -> web.Response:
    ...
    boot_text = "아직 READY 아님"
    if bot_status.ready_at:
        boot_elapsed = int((bot_status.ready_at - bot_status.started_at).total_seconds())
        boot_text = f"{boot_elapsed}초"
...
<body>
  <div class="card">
    ...
    <h1>{label}</h1>
    <p>{detail}</p>
    <small>Discord READY 부팅: {boot_text} · 마지막 실행: {last_run_text} · 30초마다 자동 새로고침</small>
  </div>
</body>
</html>"""
