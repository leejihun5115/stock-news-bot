...
class StockNewsBot(commands.Bot):
    def __init__(self, settings: Settings):
        ...
        bot_status.health_stale_threshold_seconds = settings.health_stale_threshold_seconds
    ...
    async def on_ready(self) -> None:
        ...
        boot_seconds = (bot_status.ready_at - bot_status.started_at).total_seconds() if bot_status.ready_at else 0.0
        logger.info("⚡ Discord READY까지 부팅 소요: %.2f초", boot_seconds)
    ...
    async def _send_boot_notice(self) -> None:
        ...
        message = (
            ...
            "✅ **뉴스 수집을 즉시 시작합니다.**\n"
            "`/status` · `/search-status` · `/run-now` · `/pause` · `/resume` · "
            "`/reload` · `/test-telegram` · `/데이터정리`"
        )
