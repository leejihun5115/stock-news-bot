        ...
        while True:
            ...
            try:
                ...
                if passes_deep_dive_score and llm_enabled and (has_gemini_key or has_openrouter_key):
                    ...
                    if self._llm_total_count > 0:
                        item.llm_success_rate = round(
                            self._llm_success_count / self._llm_total_count * 100
                        )
                    if llm_result:
                        if llm_result.title:
                    ...
                    # AI 심층분석(딥다이브): 상장사(company)가 확인된 뉴스에 한해
                    # 회사 핵심사업/팩트/체크포인트를 별도로 생성해 item에 채운다.
                    # 실패해도 기존 발송 흐름은 막지 않는다(그냥 필드가 비어있게 됨).
                    if item.company:
                        try:
                            deep_dive_result = await asyncio.to_thread(
                                analyze_deep_dive,
                                gemini_api_key=self.settings.gemini_api_key,
                                gemini_model=self.settings.llm_model,
                                openrouter_api_key=self.settings.openrouter_api_key,
