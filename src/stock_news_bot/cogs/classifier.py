...
def _key_points(item: NewsItem) -> list[str]:
    ...
    if item.event_type:
        points.append(f"기술수출 계약 — {item.company}" if item.company and "기술수출" in text else (f"{item.event_type} — {item.company}" if item.company else f"{item.event_type} 관련 내용"))
    if item.amounts:
...
def _analysis(item: NewsItem) -> list[str]:
    result: list[str] = []
    if item.event_type == "계약" and item.company and len(item.amounts) >= 2:
        ...
        if total and upfront and total > 0:
            ...
            result.append(
                f"{item.company} 계약규모 {item.amounts[0]}, 선급금 {item.amounts[1]}로 선급금 비중 {pct:.1f}%"
            )
    elif item.event_type == "계약" and item.company and item.amounts:
        result.append(f"{item.company} 계약규모 {item.amounts[0]}로 대형 계약 재료가 발생")
    elif item.event_type == "실적" and item.company and item.reason:
        result.append(f"{item.company} 실적 변화가 기사에 제시된 {item.reason}와 연결")
    elif item.event_type in {"임상", "허가", "승인"} and item.company and item.reason:
        result.append(f"{item.company} {item.event_type} 진행 내용이 {item.reason}로 제시")
...
def classify_item(
    ...
    item.company = extract_company_name(text)
    item.event_type = _event_type(text)
    item.theme = _theme(text)
    ...
    if item.company and item.event_type:
        item.related_companies.append((item.company, f"{item.event_type}의 직접 당사자", f"{item.event_type}에 따른 계약금·후속 기술료가 실적에 연결"))
    return item
