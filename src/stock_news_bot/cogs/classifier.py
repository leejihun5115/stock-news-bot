...
# 금액 표현 (예: "500억원", "1조 2000억원", "35,000원", "1.5조원")
# 소수점(예: "1.5") 뒤에 단위+원이 오는 경우까지 잡아야 한다.
# 이전 버전은 \d[\d,]*만 허용해 "1.5조원"에서 "1."을 못 삼키고
# 다음 자리인 "5조원"만 잘못 매칭하는 버그가 있었다.
_AMOUNT_PATTERN = re.compile(r"\d[\d,]*(?:\.\d+)?\s?(?:조|억|만)?\s?원")
