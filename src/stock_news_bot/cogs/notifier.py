...
def build_telegram_text(
    ...
    *,
    ...
    lines = [
        f"<b>📰 [{html.escape(_normalize_source(item.source))}] _{html.escape(status)}_　　⏰ {item.published_at.astimezone().strftime('%H:%M')}</b>",
        "",
    ...
    if item.related_companies:
        ...
        for name, reason, impact in item.related_companies[:5]:
            ...
            if reason:
                lines.extend(_wrap_line("　　　↳ Why : ", reason))
            if impact:
                lines.extend(_wrap_line("　　　↳ 근거 : ", impact))
    ...
    *,
    ...
    if item.related_companies:
        ...
        for name, reason, impact in item.related_companies[:5]:
            related.append("↳ " + name)
            if reason:
                related.append("　　↳ Why : " + reason)
            if impact:
                related.append("　　↳ 근거 : " + impact)
        if related:
