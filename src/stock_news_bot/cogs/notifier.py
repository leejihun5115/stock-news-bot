...
from stock_news_bot.company_profile import CompanyProfile, bilingual_company_label, resolve_company_profile, is_listed_company
...
class DetailView(discord.ui.View):
    """상세보기로 바뀐 뒤 붙는 버튼 — "🗑️ 삭제" 하나만 남긴다.

    상세는 이미 원본 뉴스 메시지를 edit_message()로 덮어쓴 상태이므로,
    삭제 버튼도 새 메시지를 보내지 않고 그 메시지 자체를 지운다.
    """
...
class TradePointView(discord.ui.View):
    ...
    아래로 갈 필요가 없음). 편집 후에는 view를 DetailView(삭제 버튼만)로
    교체한다. timeout=None으로 둬서 버튼이 시간 지나 자동 비활성화되지
