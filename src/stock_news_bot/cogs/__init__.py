...
discord.py의 코그 로드 순서는 암묵적인 의존관계를 만든다. 이 봇에서는:
  ...
  2. scheduler — 뉴스 파이프라인을 최대한 빨리 시작한다. market_intel은
     동일 SQLite 저장소에 백그라운드로 데이터를 채우므로 scheduler보다 먼저
     로드할 필요가 없다.
  3. market_intel — DART/pykrx 백그라운드 갱신. 뉴스 파이프라인과 독립적으로
     시작한다.
...
LOAD_ORDER: list[str] = [
    ...
    # 라르고TV 전용 독립 피드. scheduler의 alerter를 재사용하므로
    # scheduler보다 뒤, admin보다는 앞이면 된다.
    "stock_news_bot.cogs.largotv_feed",
