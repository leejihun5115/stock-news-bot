...
사용법:  cd ~/stock-news-bot && python3 일정_특징주_섹션_패치_v5.py
"""
...
HELPER_CODE = r'''"""일정 알림용 특징주/상한가 섹션 (patch: 일정_특징주_섹션_패치_v5).
...
def _upcoming_events(db_path, companies: list[str]) -> list[tuple[str, str, str, str]]:
    """예정 일정이 저장된 종목만 (날짜, 종목, 이벤트, 근거 기사 제목)을 날짜순으로 반환."""
    if not companies:
        return []
    ...
    try:
        with closing(sqlite3.connect(str(db_path), timeout=10)) as conn:
            rows = conn.execute(
                f"SELECT event_date, company, event_type, source_title FROM schedule_events "
    ...
    for event_date, company, event_type, title in rows:
        key = (event_date, company, event_type)
        if key in seen:
            continue
        seen.add(key)
        out.append((event_date, company, event_type, title or ""))
    return out


def _one_line(stats: dict, top_n: int, with_count: bool, skip: set[str] | None = None) -> str:
    ranked = sorted(stats.items(), key=lambda kv: (kv[1]["count"], kv[1]["last"]), reverse=True)
    names = []
    for name, info in ranked:
        if skip and name in skip:
            continue
        names.append(f"{name} {info['count']}" if with_count else name)
        if len(names) >= top_n:
            break
    return " · ".join(names)
...
def build_featured_section(db_path, days: int = 3, top_n: int = 5) -> str:
    """간단 요약: 특징주/상한가는 한 줄씩, 그 종목의 예정 일정(날짜·이벤트·근거 기사)만 상세히."""
    featured, limit_up = collect(db_path, days=days)
    if not featured and not limit_up:
        return ""
    out: list[str] = []
    if limit_up:
        out.append(f"🔺 상한가({days}일): " + _one_line(limit_up, top_n, with_count=False))
    feat = _one_line(featured, top_n, with_count=True, skip=set(limit_up))
    if feat:
        out.append(f"🔥 특징주({days}일, 기사수): " + feat)

    names = list(dict.fromkeys([*limit_up, *featured]))
    events = _upcoming_events(db_path, names)
    if events:
        out.append("📌 이 종목들의 예정 일정 (근거 기사)")
        for event_date, company, event_type, title in events[:8]:
            out.append(f"• {event_date[5:]} {company} {event_type}")
            if title:
                out.append(f"   └ {title[:45]}")
    out.append("※ 뉴스 제목 표기 기준 집계이며, 매매 추천이 아닙니다.")
    return "\n".join(out)
'''
...
def plan_patch(src: str) -> dict:
    ...
    for line in lit_lines:
        ...
        for cand in cands:
            ...
            head = [
                f"{ind}# {MARKER} 일정 알림 특징주/상한가 섹션 (일정_특징주_섹션_패치_v5)",
                f"{ind}try:",
